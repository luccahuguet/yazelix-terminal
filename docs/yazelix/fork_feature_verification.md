...
| runtime | Emoji and icon glyph fallback | Preserves emoji metadata for symbol-map font hits and maps packaged yzxterm prompt/status emoji ranges to `Noto Color Emoji` while keeping private-use icon glyphs on `Symbols Nerd Font Mono` | Prompt emoji and common status glyphs use the packaged color emoji face instead of rough text fallback, and prompt glyphs outside emoji coverage keep normal font fallback instead of being forced to emoji tofu. | Gives yzxterm a clean place to tune emoji/icon fallback quality without moving terminal-specific font logic into main Yazelix. | `rustfmt --check sugarloaf/src/font/mod.rs`; `git diff --check`; `nix-instantiate --parse pkgRio.nix`; manual fresh-window glyph screenshot evidence |
